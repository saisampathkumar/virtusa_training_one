...
export class CatalogComponent implements OnInit {
  ...
  ngOnInit() {
    this.catalogSvc.getProducts().subscribe(catalog => this.displayCatalog = catalog);
  }
  ...
  applySearch(searchText: string) {
    if (searchText === ' ') {
      ...
      this.catalogSvc.search(searchText).subscribe(catalog => this.displayCatalog = catalog, 
